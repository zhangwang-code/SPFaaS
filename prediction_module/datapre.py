...
def data_start(nm):
    path = 'D:\\AzureFunction\\newData.csv'
    dt = pd.read_csv(path, header=0)
    dt = dt.iloc[:, :4320]

    l = dt.iloc[nm, :]
    y = [0 for m in range(4320)]
    x = []
    ...
    for j in range(4320):
        if l[j] > 0:
            invoke += 1
    lambda_num = int(4320 / invoke)
    lambda_list = [1 for _ in range(4320)]
    if lambda_num >= 5:
        for j in range(4320):
            if l[j] > 0:
                for k in range(max(0, j - 4), min(4319, j + 5)):
                    if l[j] * (pow(lambda_num, (lambda_num+k-j))/math.factorial(lambda_num+k-j)*pow(math.e, -lambda_num)) < lambda_list[k]:
                        #print(lambda_num+k-j)
                        lambda_list[k] = l[j] * (pow(lambda_num, (lambda_num+k-j))/math.factorial(lambda_num+k-j)*pow(math.e, -lambda_num))
    for j in range(4320):
        x.append(j+1)
        if l[j] > 0:
            for k in range(max(0, j - 4), min(4319, j + 5)):
