...
tcn_train_features, tcn_train_target = 0, 0
train_set = 0
train_set_iter = 0
...
def main():
    ...
    # 优化器的定义S
    optim_gru = optim.Adam(params=net_gru.parameters(), lr=LEARN_RATE, weight_decay=0.0001)
    ...
    tcn_test_for_gru(tcn_train_features, net_gru, device)
...
def tcn_test_for_gru(dataset_features, net_gru, device):
    dataset_features = dataset_features.reshape([len(dataset_features), lengths, 1])
    #print(dataset_features)
    y_pred_tcn = net_gru(torch.from_numpy(dataset_features).to(device))
    y_pred_tcn = y_pred_to_numpy(y_pred_tcn)
    # y_pred = y_pred.reshape(y_pred.size/targets, targets)
    pred_tcn.extend(y_pred_tcn)
    print(len(pred_tcn))
...
path1 = 'C:\\Users\\17556\\PycharmProjects\\SPFaaS\\test.csv'
path2 = 'D:\\AzureFunction\\newData.csv'
...
if __name__ == '__main__':
    ...
    for i in range(num, num+1):
        ...
        for k in range(1, 4):
            for j in range(k):
                train_features, train_target = data_prediction_zyy(y[1440:2880], lengths, targets, k, j + 1)
                test_features, test_target = data_prediction_zyy(y[:1440], lengths, targets, k, j + 1)
                tcn_train_features, tcn_train_target = data_prediction_zyy(y[2880:], lengths, targets, k, j + 1)
            ...
            if k == 3:
                for m in range(464):
                    ...
                    predict3_tcn.append(pred_tcn[m])
                    predict3_tcn.append(pred_tcn[465 + m])
                    predict3_tcn.append(pred_tcn[929 + m])
                predict3.append(pred[465])
                predict3_tcn.append(pred_tcn[465])
                pred = []
                pred_tcn = []
            elif k == 2:
                for m in range(703):
                    ...
                    predict2_tcn.append(pred_tcn[m])
                    predict2_tcn.append(pred_tcn[704 + m])
                predict2.append(pred[704])
                predict2_tcn.append(pred_tcn[704])
                pred = []
                pred_tcn = []
            elif k == 1:
                for m in range(1440 - lengths - targets + 1):
                    predict1.append(pred[m])
                    predict1_tcn.append(pred_tcn[m])
                pred = []
                pred_tcn = []
        ...
        for p in range(43, 1436):
            ...
            for j in range(3):
                ...
                for k in range(5):
                    if j == 0:
                        sub += abs(true_list_tcn[p + k] - predict1_tcn[p - 15][k])
                    elif j == 1:
                        sub += abs(true_list_tcn[p + k] - predict2_tcn[p - 29][k])
                    elif j == 2:
                        sub += abs(true_list_tcn[p + k] - predict3_tcn[p - 43][k])
                if m > sub:
                    m = sub
                    j0 = j
            # true_index[p-15][j0] = 1
            true_index_tcn.append(j0)
        ...
        target_tcn = torch.tensor(np.array(true_index_tcn)).squeeze(-1)
        predict_tcn = []
        for i in range(1393):
            l = []
            for j in range(5):
                l.append(predict1_tcn[i+28][j])
                l.append(predict2_tcn[i+14][j])
                l.append(predict3_tcn[i][j])
            predict_tcn.append(l)
        predict_tcn = torch.tensor(predict_tcn)

        train(0, predict_tcn, target_tcn)
    ...
    for i in range(len(TCN)):
        # with open('TCNpre.csv', 'a+', encoding='UTF8', newline='') as f:
        #     writer = csv.writer(f)
        #     # write the data
        #     writer.writerow(TCN[i])
        #     print(len(TCN[i]))
